...
class User(UserMixin, db.Model):
    ...
    plano = db.Column(db.String(20), default='gratuito')  # gratuito, basico, premium
...
def visualizar_surpresa(id_unico):
    ...
    return render_template('surpresa.html', surpresa=surpresa)
...
def pagamento(plano):
    planos_disponiveis = {
        'basico': {'nome': 'Básico', 'preco': 19.90, 'creditos': 5, 'descricao': '5 surpresas românticas'},
        'premium': {'nome': 'Premium', 'preco': 39.90, 'creditos': 15, 'descricao': '15 surpresas românticas + recursos exclusivos'},
        'ilimitado': {'nome': 'Ilimitado', 'preco': 79.90, 'creditos': 999, 'descricao': 'Surpresas ilimitadas + suporte prioritário'}
    }
...
def pagamento_sucesso(plano):
    ...
    if plano in planos_disponiveis:
        ...
        pagamento = Pagamento(
            ...
            valor={'basico': 19.90, 'premium': 39.90, 'ilimitado': 79.90}[plano],
            status='aprovado'
        )
...
@login_required
def admin():
    if current_user.username != 'admin':  # Simples verificação de admin
        flash('Acesso negado!')
        return redirect(url_for('dashboard'))
